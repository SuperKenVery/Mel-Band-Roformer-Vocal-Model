...
class MelBandRoformer(Module):
    ...
    def forward(self, raw_audio, target=None, return_loss_breakdown=False):
        ...
        if masks.is_complex():
            masks_summed_real = torch.zeros_like(
                stft_repr_expanded_stems.real
            ).scatter_add_(2, scatter_indices, masks.real)
            masks_summed_imag = torch.zeros_like(
                stft_repr_expanded_stems.imag
            ).scatter_add_(2, scatter_indices, masks.imag)
